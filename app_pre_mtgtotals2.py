...
def rebuild_mtg_set_totals():
    ...
    Calls _walk_mtg_set_total directly (never the sidecar-aware
    _get_mtg_set_total) so a stale or missing sidecar can never feed back
    into itself — every run recomputes from the actual CardsDB contents.
    ...
    for set_id, meta in set_metadata.items():
        ...
        if override_game and override_game != "MTG":
            ...
            continue
        totals[set_id] = _walk_mtg_set_total(set_id)
    ...
    return {"baked": len(totals), "skipped": skipped}

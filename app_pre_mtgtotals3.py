...
def _walk_all_mtg_set_totals(set_ids):
    ...
    A real run timed out at 1800s calling _walk_mtg_set_total once per set
    (each call re-lists the ~80k-entry CardsDB/mtg directory from scratch
    against the Modal volume — 250+ sets means 250+ redundant network
    listdir()s). This lists the directory ONCE and reuses it for every
    set_id's prefix filter — the per-folder matching rule (startswith
    std_prefix/alt_prefix) and per-match logic (open profile.json, dedupe
    by lowercased name) are copied verbatim from _walk_mtg_set_total.

    Output is identical to calling _walk_mtg_set_total(set_id) once per
    set_id: no two distinct set_ids' prefixes can match the same folder
    (std_prefix always starts with "mtg-" and ends with the set_id's own
    trailing hyphen; confirmed no MTG set_id contains a hyphen, so prefixes
    don't nest), so each folder is attributed to at most one set_id either
    way — only the redundant listdir() calls are eliminated, not the
    matching semantics.

    The live per-request fallback in _get_mtg_set_total still calls
    _walk_mtg_set_total for a single not-yet-baked set, where caching the
    listing wouldn't help (only one set per call).
    """
    from vertical_loader import get_db_root as _gdbr
    cards_dir = os.path.join(_gdbr() or "CardsDB", "mtg")
    try:
        ...
        for sku_folder in all_folders:
            ...
            if os.path.exists(profile_path):
                try:
                    ...
                    pass
        totals[set_id] = len(names) if names else None
    return totals
...
def sets_cards(set_id):
    ...
    # Infer game from set_id if not in metadata
    if not game:
    ...
    if game in ("POKEMON", "MTG"):
        ...
        for row in rows:
            ...
            prof = _load_card_profile_for_sku(sku, db_root, get_data_dir())
    ...
    # Sort numerically by card_number where possible

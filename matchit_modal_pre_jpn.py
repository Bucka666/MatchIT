            ...
            for game_folder in ("pokemon", "mtg", "yugioh"):
                ...
                for sku in os.listdir(game_dir):
                    if sku.startswith("_"):
                        continue
                    parts = sku.split("-")
                    if game_folder == "pokemon" and parts[0] in new_set_ids:
                        new_skus.append(sku)
        ...
        for game_folder in ("pokemon", "mtg", "yugioh"):
            ...
            for name in os.listdir(game_dir):
                if name.startswith("_"):
                    continue
                parts = name.split("-")
                if game_folder == "pokemon" and len(parts) >= 2 and parts[0].lower() in target_ids:
        ...
        for game_folder, game in game_label.items():
            ...
            for name in os.listdir(game_dir):
                ...
                if game == "POKEMON" and "-" in name:
                    sets_by_game["POKEMON"].add(parts[0])
...
@app.local_entrypoint()
def rebuild_new_sets():
    rebuild_lookup_files.remote(new_set_ids=["me4", "hob", "fra"])

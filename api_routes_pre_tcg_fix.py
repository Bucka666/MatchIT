    ...
    def api_match():
        ...
        if results:
            ...
            if not _effective_tcg and results:
                ...
                if _top_sku.startswith("ygo-"):
                    ...
                    _effective_tcg = "MTG"
                elif _top_sku.startswith(("sv", "swsh", "base", "neo", "dp", "bw", "xy", "sm", "me", "pl", "ex")):
        ...
        for i, r in enumerate(results[:top_k]):
            ...
            if sku in sku_profile_map:
                match_entry["profile"] = sku_profile_map[sku]
        ...
        try:
            from app import _record_price, _extract_gbp_from_profile, _increment_scan_counter
            if matches:
                _m0 = matches[0]

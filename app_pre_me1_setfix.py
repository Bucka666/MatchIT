...
def _get_mtg_set_total(set_id):
    ...
    try:
        ...
        pass
    result = len(names) if names else None
...
def sets_completion():
    ...
    for item in items:
        ...
        # Cross-check inferred game against set_metadata (catches me1, me2pt5 etc.)
        meta_game = set_metadata.get(set_id, {}).get("game")
        if meta_game and meta_game != game:
            game = meta_game

        # Verify SKU actually belongs to this game (prevents bare-prefix collisions)
        if _get_sku_game(sku) != game:
            continue
        ...
        if set_id not in sets_data:
            ...
            sets_data[set_id] = {
                "set_id": set_id,
                "name": meta.get("name") or item.get("set_name") or set_id,
                "game": game,
                "owned_skus": set(),
                "owned_names": set(),
...
def match():
    ...
    try:
        ...
        print(f"[TIMING2] ocr_block={_t_post_start - _t_ocr_start:.2f}s (incl profile enrich + ocr)", flush=True)
    except Exception:
        pass
    _increment_scan_counter()

    # Rule-based condition grade from query image
    _t_grade_start = _time.time()
    grade = _safe_grade(str(query_path1))
    _t_render_start = _time.time()
    print(f"[TIMING2] post_write={_t_grade_start - _t_post_start:.2f}s  grade={_t_render_start - _t_grade_start:.2f}s", flush=True)

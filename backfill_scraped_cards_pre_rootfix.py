...
def register_scraped_cards(
    ...
    """
    db_path, img_dir, cards_root = _get_paths()
...
def _run_remote(tcg: str, sets, dry_run: bool):
    import os, sys
    os.environ["LOCALAPPDATA"] = "/modal_data"
    sys.path.insert(0, "/app")
    from backfill_scraped_cards import register_scraped_cards as _reg
    return _reg(tcg=tcg, sets=sets, dry_run=dry_run)


@app.local_entrypoint()
def main(tcg: str = "all", sets: str = "", dry_run: bool = False):
    """
    Run with:
    ...
    _run_remote.remote(tcg=tcg, sets=sets_list, dry_run=dry_run)

...
runs incremental embedding, and sends an email summary.
...
Modal integration (add to matchit_modal.py):

    @app.function(
        image=image,
        gpu="T4",
        volumes={"/modal_data": vol},
        schedule=modal.Cron("0 3 * * 1"),  # Every Monday at 3am UTC
        timeout=3600,
    )
...
def _detect_new_pokemon_sets() -> List[Dict]:
    """
...
def run_scheduler(
    ...
    logger.info(f"[SCHED] Starting scheduler run at {run_at} — TCGs: {tcgs}")

    any_new = False

    # ── 1. Detect + download per TCG ──
    for tcg in tcgs:
        tcg = tcg.upper()
        ...
        try:
            if tcg == "POKEMON":
                new_sets = _detect_new_pokemon_sets()
            elif tcg == "MTG":
    ...
    # Runs every week regardless of any_new, since profile.json corrections
    ...
    # Same "runs every week regardless of any_new" rationale as 2c.
    if not dry_run:
        try:

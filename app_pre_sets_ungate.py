    ...
    try:
        ...
        if game in ('pokemon', 'mtg'):
            if identifier and '-' not in str(identifier):
                print(f'[OCR-LOOKUP] Bare number rejected: {identifier}', flush=True)
        ...
        if not sku:
            print(f'[OCR-LOOKUP] No SKU for: {identifier}', flush=True)
            return jsonify({'status': 'not_found'})
        ...
        print(f'[OCR-LOOKUP] Hit: {identifier} -> {sku}', flush=True)
        return jsonify({
            'status': 'ok',
            'source': 'ocr_lookup',
            'sku': sku,

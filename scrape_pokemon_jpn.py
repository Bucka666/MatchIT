...
def _justtcg_get_with_backoff(url: str, headers: dict, timeout: int, max_retries: int = 5):
    """GET with Retry-After-aware backoff on 429.
    Honors the Retry-After response header if present; falls back to a 60s
    fixed delay if not — ensuring at least one full rate-limit window clears
    before retrying. Previous 1s/2s/4s/8s/16s schedule exhausted all retries
    within ~15s, never clearing a per-minute limit.
    ...
    for attempt in range(max_retries):
        try:
                ...
                if attempt < max_retries - 1:
                    ...
                    if retry_after is not None:
                        try:
                            ...
                            print(f"[JUSTTCG] 429 — honoring Retry-After: {delay}s (attempt {attempt + 1}/{max_retries})")
                        except ValueError:
                            delay = _FALLBACK_DELAY
                            print(f"[JUSTTCG] 429 — unparseable Retry-After '{retry_after}', using fallback {delay}s (attempt {attempt + 1}/{max_retries})")
                    else:
                        delay = _FALLBACK_DELAY
                        print(f"[JUSTTCG] 429 — no Retry-After header, using fallback {delay}s (attempt {attempt + 1}/{max_retries})")
                    time.sleep(delay)
                    continue
            ...
            if attempt < max_retries - 1:
                print(f"[JUSTTCG] Request error, retrying in {_FALLBACK_DELAY}s (attempt {attempt + 1}/{max_retries}): {e}")
